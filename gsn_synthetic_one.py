...
from synthetic_one import SyntheticOne
mpl.use('Agg')
...
W1 = get_shared_weights(1, hidden_size, numpy.sqrt(6. / (2 + hidden_size)))
W2 = get_shared_weights(hidden_size, hidden_size, numpy.sqrt(6. / (hidden_size + hidden_size)))
b0 = get_shared_bias(1)
...
for i in range(n_epoch):
  ...
  if (i+1)%3 == 0:
    ...
    plt.clf()
    plt.hist(samples[:,0,0], 50, normed=True)
    print(len(samples[:,0,0]))
    plt.savefig('gsn_synth_one' + str(i) + '.png')
