...
for i in range(walkbacks):
  ...
  # Activate
  h1 = tf.sigmoid(tf.matmul(x_corrupt, W1) + tf.matmul(h2, tf.transpose(W2)) + b1)
  # Activate
  h2 = add_gaussian_noise(tf.sigmoid(add_gaussian_noise(tf.matmul(h1, W2) + b2, hidden_add_noise_sigma)), hidden_add_noise_sigma)
  # Activate
